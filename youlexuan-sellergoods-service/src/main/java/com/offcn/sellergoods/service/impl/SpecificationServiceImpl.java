...
public class SpecificationServiceImpl implements SpecificationService {
	...
	public void update(Specification specification){
		...
		//修改多  先全部删除  再添加
		//1 根据规格id删除所有
		TbSpecificationOptionExample exampale= new TbSpecificationOptionExample();
		TbSpecificationOptionExample.Criteria criteria = exampale.createCriteria();
		criteria.andSpecIdEqualTo(specification.getSpecification().getId());
		specificationOptionMapper.deleteByExample(exampale);
		//2 插入
		List<TbSpecificationOption> specificationOptionList = specification.getSpecificationOptionList();
		for (TbSpecificationOption specificationOption : specificationOptionList) {
			//本次插入的规格id获取到set到规格项中 直接从规格中获取  但是默认插入是不返回id的  需要从mapper中设置
	...
	public Specification findOne(Long id){
		...
		// 上面这句话翻译成就是 select *from tb_specification where id =39
		...
		// 上面这句翻译成就是  select * from tb_specification_option where spec_id=39
